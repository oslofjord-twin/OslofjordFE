    ...
    return (
        <div className='absolute top-3 z-10'>
            ...
            <div className=' bg-slate-100 w-80 max-h-56 min-h-fit overflow-y-scroll mt-2 rounded scroll-smooth cursor-pointer border border-slate-600' id="dropdownmenu">
                <ul className=' divide-y divide-slate-400 cursor-pointer ' >
                    {filteredSpecies.map((species) => (
                        <DropdownItem setChosenSpecies={setChosenSpecies} setDropdown={setDropdown} key={species.id} itemId={species.id} name={species.name}></DropdownItem>
                    ))}
